...
class SupplierService {
  ...
  async addSupplier(request: AddSupplierRequest): Promise<SupplierResponse> {
    return await httpClient.post<SupplierResponse>('/api/suppliers', request);
  }

  // Update an existing supplier
  async updateSupplier(supplierId: number, request: UpdateSupplierRequest): Promise<SupplierResponse> {
    return await httpClient.put<SupplierResponse>(`/api/suppliers/${supplierId}`, request);
  }

  // Get supplier by ID
  async getSupplierById(supplierId: number): Promise<SupplierResponse> {
    return await httpClient.get<SupplierResponse>(`/api/suppliers/${supplierId}`);
  }

  // Get all suppliers
  async getAllSuppliers(): Promise<SupplierResponse[]> {
    return await httpClient.get<SupplierResponse[]>('/api/suppliers');
  }

  // Get all active suppliers
  async getActiveSuppliers(): Promise<SupplierResponse[]> {
    return await httpClient.get<SupplierResponse[]>('/api/suppliers/active');
  }

  // Get all inactive suppliers
  async getInactiveSuppliers(): Promise<SupplierResponse[]> {
    return await httpClient.get<SupplierResponse[]>('/api/suppliers/inactive');
  }

  // Get supplier by email
  async getSupplierByEmail(email: string): Promise<SupplierResponse> {
    return await httpClient.get<SupplierResponse>(`/api/suppliers/email/${encodeURIComponent(email)}`);
  }

  // Get supplier by company name
  async getSupplierByCompanyName(companyName: string): Promise<SupplierResponse> {
    return await httpClient.get<SupplierResponse>(`/api/suppliers/company/${encodeURIComponent(companyName)}`);
  }
  ...
  async searchSuppliers(searchRequest: SearchSupplierRequest): Promise<SupplierResponse[]> {
    ...
    return await httpClient.get<SupplierResponse[]>(`/api/suppliers/search?${params.toString()}`);
  }

  // Delete a supplier
  async deleteSupplier(supplierId: number): Promise<void> {
    return await httpClient.delete<void>(`/api/suppliers/${supplierId}`);
  }

  // Activate a supplier
  async activateSupplier(supplierId: number): Promise<SupplierResponse> {
    return await httpClient.patch<SupplierResponse>(`/api/suppliers/${supplierId}/activate`, {});
  }

  // Deactivate a supplier
  async deactivateSupplier(supplierId: number): Promise<SupplierResponse> {
    return await httpClient.patch<SupplierResponse>(`/api/suppliers/${supplierId}/deactivate`, {});
  }
}
