...
class SalesService {
  ...
  async getTotalSalesAmountBetweenDates(startDate: string, endDate: string): Promise<number> {
    try {
      return await httpClient.get<number>(`/api/sales/analytics/total-amount?startDate=${startDate}&endDate=${endDate}`);
    } catch (error) {
  ...
  async getMedicineWiseSalesQuantity(): Promise<MedicineWiseSales[]> {
    try {
      const data = await httpClient.get<unknown[]>('/api/sales/analytics/medicine-wise-sales');
      // Transform the Object[] response to MedicineWiseSales[]
      return data.map((item: unknown) => {
        const itemArray = item as [string, number];
        return {
          medicineName: itemArray[0],
          totalQuantity: itemArray[1]
        };
      });
    } catch (error) {
